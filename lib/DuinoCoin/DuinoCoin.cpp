...
bool DuinoCoin::loop(void) {
  if (serial.available() > 0) {
    ...
    unsigned int ducos1result =
        ducos1a(lastblockhash, newblockhash, difficulty);
...
// DUCO-S1A hasher
uint32_t DuinoCoin::ducos1a(String lastblockhash, String newblockhash,
                            int difficulty) {
...
String DuinoCoin::getDUCOID() {
  ...
  for (size_t i = 0; i < 4; i++) {
    sprintf(buff, "%02X", (uint8_t)serialNumber[i]);
    ID += buff;
  }
  return ID;
}
